...
def Compute_tda_feature(args):
    ...
    return [file_name.replace('.npy', ''), feature_vectors]

def get_patient_ID(file_name):
    match = re.match(r'^(.*?)_g+', file_name)
    if match:
...
if __name__ == '__main__':
    ...
    with tqdm(classes) as classes_tqdm:
        for class_name in classes_tqdm:
            classes_tqdm.set_description(f'Dataset: {class_name}')
            
            pds_path = f'..\\persistent_diagrams\\{class_name}_pds_dict.npy'
            ...
            main_feat_dir = f'extracted_features\\{class_name}'
